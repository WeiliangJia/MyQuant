"""
策略 V4：33 因子（25 股票 + 8 跨资产）+ LightGBM + Ridge + 降换手
==================================================================
在 V3 降换手基础上，增加 8 个跨资产因子（VIX、利率、美元指数）。
"""
...
PREDICTIONS_PATH = SCRIPT_DIR / "predictions_v4.parquet"
HOLDINGS_PATH = SCRIPT_DIR / "holdings_v4.parquet"
REPORT_PATH = SCRIPT_DIR / "strategy_v4_report.json"
...
def rolling_train_predict(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    print(f"\n── Feature Importance (all {len(FEATURE_COLS)} features) ──")
...
def main() -> None:
    print("=" * 60)
    print("  STRATEGY V4: 33 Factors + Turnover Control")
    print("=" * 60)
    ...
    # V3 vs V4 comparison
    v3_report_path = SCRIPT_DIR / "strategy_v3_report.json"
    comparison = {}
    if v3_report_path.exists():
        v3 = json.loads(v3_report_path.read_text(encoding="utf-8"))
        v3_lgb = v3.get("results", {}).get("LightGBM", {})
        v4_lgb = results.get("LightGBM", {})
        if v3_lgb and v4_lgb:
            comparison = {
                "V3_sharpe": v3_lgb.get("sharpe_ratio"),
                "V4_sharpe": v4_lgb.get("sharpe_ratio"),
                "V3_return": v3_lgb.get("annual_return"),
                "V4_return": v4_lgb.get("annual_return"),
                "V3_mean_ic": v3_lgb.get("mean_ic"),
                "V4_mean_ic": v4_lgb.get("mean_ic"),
                "V3_features": 21,
                "V4_features": len(FEATURE_COLS),
            }
            print(f"\n── V3 (21 features) vs V4 (29 features) ──")
            print(f"  Sharpe:  {comparison['V3_sharpe']} → {comparison['V4_sharpe']}")
            print(f"  Return:  {comparison['V3_return']} → {comparison['V4_return']}")
            print(f"  IC:      {comparison['V3_mean_ic']} → {comparison['V4_mean_ic']}")
